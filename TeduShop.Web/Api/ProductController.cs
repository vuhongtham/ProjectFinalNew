    ...
    public class ProductController : ApiControllerBase
    {
        ...
        public async Task<HttpResponseMessage> ExportPdf(HttpRequestMessage request, int id)
        {
            ...
            try
            {
                ...
                replaces.Add("{{Description}}", product.Content);
                replaces.Add("{{Warranty}}", product.Warranty + " tháng");
                replaces.Add("{{Quantity}}", product.Quantity + " sản phẩm");
        ...
        private List<Product> ReadProductFromExcel(string fullPath, int categoryId)
        {
            using (var package = new ExcelPackage(new FileInfo(fullPath)))
            {
                ...
                for (int i = workSheet.Dimension.Start.Row + 1; i <= workSheet.Dimension.End.Row; i++)
                {
                    ...
                    //Mô tả
                    productViewModel.Content = workSheet.Cells[i, 2].Value.ToString();
                    ...
                    //Mô tả 2
                    productViewModel.Description = workSheet.Cells[i, 8].Value.ToString();
